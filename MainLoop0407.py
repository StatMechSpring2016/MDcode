...
v = sqrt(K_B*targetT*N_Particles/Mass_Argon)/(N_Particles**.5)

for i in range (0,N_Particles):
	vx_Array = np.append(vx_Array,v*pow(-1,i))
	vy_Array = np.append(vy_Array,v*pow(-1,i))
	vz_Array = np.append(vy_Array,v*pow(-1,i))
...
for t in range (0,N_time_step):
	...
	if t != 0:
		if sysT > targetT +0.1 or sysT < targetT -0.1:
			...
			vx_Array.fill(vx_Array*sqrt(targetT/sysT))
			vy_Array.fill(vy_Array*sqrt(targetT/sysT))
			vz_Array.fill(vz_Array*sqrt(targetT/sysT))
